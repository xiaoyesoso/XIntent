...
> [English Technical Principles](TECHNICAL_PRINCIPLES.md)
>
> 相关文档：[HTTP API 接口文档](API.zh-CN.md)

本文档详细阐述 XIntent 框架两个模块的技术原理、设计思想与理论基础 —— **用户输入规范化**（第 1-13 节）与 **三层意图识别**（第 14-22 节）。每个原理都标注了对应的**服务接口**和**代码实现**。
...
- **服务入口**：`POST /normalize`（详见 [API 文档](API.zh-CN.md#3-post-normalize--用户输入规范化)）
...
- **API 响应字段**：`classification_tags`（详见 [API 文档 - classification_tags 可选值](API.zh-CN.md#classification_tags-可选值)）
...
- **API 响应字段**：`pronoun_resolutions`（详见 [API 文档](API.zh-CN.md#pronoun_resolutions-元素结构)）
...
- **API 响应字段**：`paused_for_clarification`、`clarification`（详见 [API 文档](API.zh-CN.md#clarification-结构)）
...
- **API 响应字段**：`term_mappings`（详见 [API 文档](API.zh-CN.md#term_mappings-元素结构)）
...
- **API 响应字段**：`quantifiable_adjectives`（详见 [API 文档](API.zh-CN.md#quantifiable_adjectives-元素结构)）
...
- **服务入口**：`POST /recognize`（详见 [API 文档](API.zh-CN.md#4-post-recognize--意图识别)）
