...
这文档我不想写成那种你翻两页就困的参考手册。

我想写的，是一个把这套系统从零到一搭起来的人，坐在你对面，喝着咖啡，跟你聊他为什么要这么设计，中间踩过什么坑，哪些地方让他自己都觉得有点得意。

如果你正在搞 Agent，意图识别又总是不稳，那你来对地方了。

XIntent 框架就两个模块，各回答一个根本问题。

**用户输入规范化**（第 1-13 节），回答的是，为什么 Agent 听不懂用户在说话？因为自然语言天生就是乱的，这模块在意图识别之前先把输入收拾干净。

**三层意图识别**（第 14-22 节），回答的是，为什么 Agent 总是选错意图？因为单次大模型调用既不够快也不够稳，这模块用瀑布式架构去平衡成本、速度和准确率。

**准确率提升**（D17-D25），回答的是，怎么把准确率从 85% 推到 99%？八个可选扩展，每个针对一类特定失败模式。

**面试洞察**（D26-D31），回答的是，面试时怎么讲这个系统？六个来自真实面试反馈的设计决策，覆盖证据分级、边界原则、结构化输出契约。
...
- [1. 为什么 Agent 总是听不懂你在说什么](#1-为什么-agent-总是听不懂你在说什么)
- [2. 用户输入的六种「病」，每种都有语言学根子](#2-用户输入的六种病每种都有语言学根子)
- [3. 两阶段流水线，一个看似简单但很关键的设计决策](#3-两阶段流水线一个看似简单但很关键的设计决策)
- [4. 大模型为什么会「越权」，以及怎么管住它](#4-大模型为什么会越权以及怎么管住它)
- [5. 指代消解与跨轮记忆，让 Agent 真正记住你说过什么](#5-指代消解与跨轮记忆让-agent-真正记住你说过什么)
- [6. 按名索引，为什么「TCC 方案」比「方案二」好](#6-按名索引为什么tcc-方案比方案二好)
- [7. 澄清机制，宁可问一句也不要瞎猜](#7-澄清机制宁可问一句也不要瞎猜)
- [8. 属性检索指代消解，给 Agent 装上 RAG 长期记忆](#8-属性检索指代消解给-agent-装上-rag-长期记忆)
- [9. 词汇表自我演进，让知识从对话中「长」出来](#9-词汇表自我演进让知识从对话中长出来)
- [10. 形容词可量化，把「性价比」变成机器能懂的数字](#10-形容词可量化把性价比变成机器能懂的数字)
- [11. 三层上下文整合，所有规范化措施的一句话总结](#11-三层上下文整合所有规范化措施的一句话总结)
- [12. 完整性校验，给规范化装一个守门员](#12-完整性校验给规范化装一个守门员)
- [13. few-shot 检索注入，让系统越用越聪明](#13-few-shot-检索注入让系统越用越聪明)

### 模块二，三层意图识别

- [14. 为什么「直接调大模型做意图识别」是错的](#14-为什么直接调大模型做意图识别是错的)
- [15. 三层瀑布式架构，从便宜到贵的正确升级姿势](#15-三层瀑布式架构从便宜到贵的正确升级姿势)
- [16. 置信度路由与多信号融合，别只信大模型自己说的](#16-置信度路由与多信号融合别只信大模型自己说的)
- [17. 槽位填充，硬约束、软约束和跨轮累积](#17-槽位填充硬约束软约束和跨轮累积)
- [18. 拒识与澄清，两种截然不同的失败要分开处理](#18-拒识与澄清两种截然不同的失败要分开处理)
- [19. 下一轮隐式评估，不问用户也能知道对不对](#19-下一轮隐式评估不问用户也能知道对不对)
- [20. 意图边界定义与 few-shot 注入，给意图画清楚地盘](#20-意图边界定义与-few-shot-注入给意图画清楚地盘)
- [21. 分层意图与决策树，100 个意图怎么塞进 Prompt](#21-分层意图与决策树100-个意图怎么塞进-prompt)
- [22. 评估指标，4 层指标体系与 9 场景测试集](#22-评估指标4-层指标体系与-9-场景测试集)

### 扩展章节

- [准确率提升 (D17-D25)，从 85% 到 99% 的八把刀](#准确率提升-d17-d25从-85-到-99-的八把刀)
- [面试洞察（D26-D31），六个来自真实面试反馈的设计决策](#面试洞察d26-d31六个来自真实面试反馈的设计决策)

---

## 1. 为什么 Agent 总是听不懂你在说什么
...
这在规范化阶段是灾难性的。规范化的职责是**理解输入**，不是**回答问题**。你想想看，如果需求分析师还没把需求整理清楚，开发就开始写代码了，那能对吗？
...
## 8. 属性检索指代消解，给 Agent 装上 RAG 长期记忆
...
**L1 代码层**（D3），三种识别方式，页面引导（UI 事件到意图）、关键字/正则匹配、规则引擎（上下文状态 + 规范化输入）。返回第一个命中或「未命中」信号。**零 LLM 调用，< 10ms。**
...
## 20. 意图边界定义与 few-shot 注入，给意图画清楚地盘
...
## 21. 分层意图与决策树，100 个意图怎么塞进 Prompt
...
## 准确率提升 (D17-D25)，从 85% 到 99% 的八把刀

回到意图识别这块，三层瀑布已经能解决大部分问题。但生产环境中你会发现，总有那么一些边界 case，瀑布搞不定。

我们花了大量时间分析生产环境的失败案例，最后归纳出五类准确率问题。D17 到 D24 就是针对这五类问题的八把刀，每把刀切一类毛病。

全部默认关闭，因为它们都有成本。你需要哪把开哪把，或者用 `accuracy_preset` 一键开一组。
...
你想想看，意图空间一大，L2 的 Prompt 里候选意图列表就长，大模型看着一堆选项容易犯迷糊。D17 的思路是，在 LLM 分类之前先用检索把候选收窄到 Top-N。
...
静态 few-shot 是固定的，对所有输入都注入同一批例子。但有些输入比较特殊，静态例子不够用。D18 的思路是，除了静态 few-shot，再动态检索一批和当前输入最相似的历史例子注入进去。

用 Jaccard token 重叠度从 `FewShotStore` 中查找。这个和 D17 的检索思路类似，但检索的是 few-shot 例子而不是候选意图。
...
意图定义之间会有重叠，两个意图的正样本太像了，大模型分不清。D19 检测意图定义之间的重叠（相似度 > 阈值），并提供 `split_intent()` 和 `merge_with_param()` 操作让你维护一个干净、正交的意图空间。
...
D20 区分真多意图请求与流程描述，返回子意图之间的依赖关系以及拓扑排序后的待执行列表。这样你就知道先做什么后做什么，而不是一股脑全扔给用户。
...
D21 的解法是，L1 没命中时回退到向量相似度匹配。用 128 维哈希向量化器（纯 Python 实现，无 numpy 依赖）和余弦相似度，在预建的向量映射里找最相似的意图。
...
D22 直接复用上一轮的意图（confidence=1.0, source="reused"），跳过整个瀑布。但在出现隐式失败信号（D11）或意图切换标记时回滚，重新跑瀑布。
...
单一识别器不确定的时候怎么办？D23 的思路是，并行跑多个识别器（向量、规则、轻量 LLM），然后投票或加权评分做仲裁。
...
当通用模型不够用的时候，你需要领域专用模型。D24 提供训练数据导出流水线（JSONL 格式）和 `model_tier="fine_tuned"` LLM 客户端层级，读取 `FINE_TUNED_MODEL` 环境变量。
...
## 面试洞察（D26-D31），六个来自真实面试反馈的设计决策

顺着上面的再聊聊。

D17-D25 是我们在生产环境中摸爬滚打总结出来的。D26-D31 不一样，这六个设计决策来自面试反馈。

我们拿这个系统去面试，面试官问的问题、课后习题的思路、复用模板里的写法，反过来帮我们发现了很多之前没想到的设计盲点。这六项默认全部开启，因为它们是增量式的，丰富输出而不改变既有行为。

### D26，证据分级，每个槽位值都要有「出处」

每个槽位值和事实都标注证据等级，这是 D26 的核心。
...
### D27，sub_tasks vs independent_intents 边界，意图的职责是选择业务流程
...
### D28，五因子路由仲裁，置信度是信号不是概率

置信度是**信号**，不是概率。这句话我在面试的时候反复强调。
